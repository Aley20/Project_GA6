...
and execute_instr env program instr current_line =
  ...
  | Assign (v, e) -> (* Intruction pour les variables (X,Y, I, ...) *)
    ...
    let next_line = find_next_line program current_line in
    execute_program env program (get_line_num next_line) (* Passe à la ligne suivante *)
  | MultiAssign (vars, exprs) -> (* Instruction pour l'extension 2 *)
    (* Vérifie si le nombre de variables et d'expressions est égal *)
    if List.length vars <> List.length exprs then
      failwith "Number of variables and expressions must match";
    (* Évalue chaque expression dans la liste des expressions *)
    let values = List.map (eval_expr env) exprs in
    (* Associe chaque variable avec sa valeur évaluée correspondante et met à jour l'environnement *)
    List.iter2 (fun v value -> Hashtbl.replace env v value) vars values;
    let next_line = find_next_line program current_line in
